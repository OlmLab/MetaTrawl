...
def _resolve_matrix_compare_options(
    ...
    *,
    ...
    resolved_memory_limit_gb = (
        ...
        else matrix_config.memory_limit_gb or 16.0
    )
    return (
...
@cli.group("view")
def view_group() -> None:
    """Explore generated MetaTrawl analysis bundles."""


@view_group.command("genomes")
@click.option(
    "--view-dir",
    required=True,
    type=click.Path(path_type=Path),
    help="Directory generated by sync-genome-views.",
)
@click.option("--host", default="127.0.0.1", show_default=True, help="Interface on which to serve the viewer.")
@click.option("--port", default=8766, show_default=True, type=click.IntRange(min=0, max=65535))
@click.option("--open/--no-open", "open_browser", default=True, show_default=True, help="Open the viewer in the default browser.")
def view_genomes(view_dir: Path, host: str, port: int, open_browser: bool) -> None:
    """Serve the interactive genome atlas generated by sync-genome-views."""
    try:
        server = viewer.create_genome_view_server(view_dir, host=host, port=port)
    except (FileNotFoundError, OSError, RuntimeError, ValueError) as exc:
        raise click.ClickException(str(exc)) from exc
    actual_host, actual_port = server.server_address[:2]
    browser_host = "127.0.0.1" if actual_host in {"0.0.0.0", "::"} else actual_host
    url = f"http://{browser_host}:{actual_port}/"
    click.echo(f"METATRAWL step=view-genomes status=ready url={url} view_dir={Path(view_dir).resolve()}")
    click.echo("Press Ctrl-C to stop the viewer.")
    if open_browser:
        click.launch(url)
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        click.echo("\nMETATRAWL step=view-genomes status=stopped")
    finally:
        server.server_close()
